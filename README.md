...
**听见每一个自己 · Explore the many perspectives within**

让代表你内心不同声音的 AI 角色，围绕你的困境**当面辩论**；你不下场、只旁观——<br/>
最终拿到的不是一个答案，而是一张属于你的「内心冲突地图」。
...
![LLM](https://img.shields.io/badge/LLM-deepseek--v4--flash-4D6BFE)
![Streaming](https://img.shields.io/badge/Streaming-SSE-FF6B35)

<br/>

[🌌 这是什么](#-这是什么) · [💡 想法的诞生](#-想法的诞生) · [🧠 心理学依据](#-心理学依据) · [🎭 运作机制](#-它是怎么运作的) · [🏗️ 架构](#%EF%B8%8F-技术选型与架构) · [✨ 独特之处](#-独特之处设计--工程) · [🚀 快速开始](#-快速开始)
...
```mermaid
flowchart LR
    A["🗣️ 对话<br/>把乱麻理成冲突地图"] --> B["⚖️ 复杂度评估<br/>L1 / L2 / L3"]
    B --> C["🔥 辩论<br/>内心声音当面交锋"]
    C --> D["🗺️ 综合图景<br/>张力星图"]
    D --> E["👤 内心画像<br/>多声部肖像"]
```
...
| 理论 | 核心命题 | 在产品里落地为 |
|---|---|---|
| 🗣️ **对话性自我理论**<br/>DST · Hermans 2001 · 被引 1,343+ | 自我是多个"我位置"（I-positions）构成的"心灵的社会"；只有每个位置被承认而非压制，对话才发生、自我才更新 | 每个角色 = 一个合法的"我位置"；辩论 = 我位置之间的对话；"不让任何声音垄断或沉默"落地为抢麦式发言治理 |
| 🏠 **内在家庭系统**<br/>IFS · Schwartz 1995 | 心智由多个"部分"（parts）+ 核心"自我（Self）"组成；**所有部分都是善意的**，目标是和谐而非消灭 | 角色 = 部分，**你本人 = Self**；"所有声音皆善意、不评判""给图不给答案、决定权在你"全部源于此 |
| 🔭 **自我距离化**<br/>Kross 2014 · 7 项研究 / N=585 | 用第三人称谈论自己的困境能创造"自我距离"，显著增强情绪调节、减少反刍 | 你旁观角色们用第三人称讨论你的困境，正是自我距离化的产品化；"不给建议、拉回感受"也以此为据 |
| 🪞 **多重自我（双重验证）**<br/>Suszek 2024 · Dulberg 2023, PNAS | 心理侧：激活"多重自我模式"即可提升开放性与认知灵活性（d = 0.31–0.44，N=989）；计算侧：多专门化模块优于单一整体智能体 | 一面证明"**仅仅展示多元视角，本身就是干预**"；一面证明"**多角色不是权宜之计，而是更优架构**" |

### 三根支撑性支柱

- 🐢 **反思性设计** —— 慢技术（可暂停、不催促）、陌生化与"有益不适"（批判审视者的理论来源）、R0–R4 反思阶梯。
- 👁️ **旁观式交互** —— 第三人称视角增强反思与自我慈悲；本项目取"**认知第三人称**"：无需 VR，在屏幕上看自己的内心被外化。
- ⚔️ **多智能体辩论** —— 多轮辩论提升推理（Du 2023）；选择性辩论深度 iMAD：token −92%、准确率 +13.5%（Fan 2025）；**多样性是辩论成功的主导因素**（Wu 2025）。

<details>
<summary>📋 <b>理论 → 设计 映射表（点击展开）</b></summary>
<br/>
...
| # | 机制 | 做法 |
|---|---|---|
| 1️⃣ | **循序渐进** | 事实处境 → 各选择的代价与担忧 → "这对你意味着什么、你想成为谁"，三层递进 |
| 2️⃣ | **只用你的原话** | 每次追问必须抓住你刚说过的一句话当落点；模型脑补情节会被拦下重问 |
| 3️⃣ | **固定纠偏动作** | 说空话→问实它；索要建议→拉回两股力量各自在怕什么；只说一头→请你自己说出另一头 |
| 4️⃣ | **换话题靠接不靠切** | 先接住你这句，再自然带出下一条，绝不审讯式跳题 |
| 5️⃣ | **知道什么时候够了** | 持续判断素材是否够撑起一场辩论，够了就主动收尾 |

**产出**：一张冲突地图——几个内心声音（关切/恐惧/语气）、价值冲突、情绪、被牵动的人。

### ⚖️ Step 2 · 复杂度评估：你的困境值得多大的阵仗
...
### 🔥 Step 3 · 辩论：让内心声音当面吵给你看

**难题**：角色要吵得**有差异**（不能一个味儿）、不能**各说各话**（跑题）、不能**互相吹捧**（回声室），最后还得给**图**而不是**答案**。七个机制：

<table>
<tr>
<td width="50%"><img src="image/04-debate-round1.png" alt="第 1 轮 — 开场陈述"/><p align="center"><sub>第 1 轮 · 开场陈述（互相不可见）</sub></p></td>
<td width="50%"><img src="image/05-debate-round2.png" alt="第 2 轮 — 正面交锋"/><p align="center"><sub>第 2 轮 · 正面交锋（实时流式字幕）</sub></p></td>
</tr>
</table>
...
7. 🗺️ **结尾给图不给答案** —— 综合者不站队，摆出核心张力、各方主张与共识地带。**决定权始终在你手里——你才是那个 Self。**
...
| 🗣️ **services/ 对话** | `elicitation` · `tension_tracker`（张力两极追踪）· `intent_planner` + `elicitation_moves`（动作调度）· `depth_evaluator` |
| 🔥 **services/ 辩论** | `orchestrator`（编排）· `spine`（主线）· `round_evaluator`（轮次评审）· `exchange_budget`（抢麦预算）· `consistency`（三层人设一致性）· `repetition`（去重）· `language_rules`（语言闸）· `memory_manager`（双层记忆）· `termination`（收敛）· `model_router`（模型分层） |
| 🗺️ **services/ 产出** | `synthesis` · `portrait_composer` · `outcome_extractor` · `report` |
| 🧯 **services/ 横切** | `llm`（统一调用）· `safety`（危机监测）· `turn_auditor`（话轮出口审计）· `embedding` · `file_store`（会话落盘） |
...
| 立场 | 说明 |
|---|---|
| 👁️ **旁观而非参与** | 你不是在和 AI 聊天，而是看自己的内心被外化、被辩论——认知第三人称，无需 VR |
| 🗺️ **给图不给答案** | 终点是冲突地图。产品帮你看清，不替你决定 |
| 💊 **产品即干预** | 按多重自我研究，把多元视角摆出来这个动作本身就有心理价值，不必强求结论 |
| 🏛️ **辩论是一个「场」** | three.js 渲染的暗色议事厅、围坐的声部、实时字幕与立场卡——氛围本身在说"这是一场关于你的严肃审议"，而不是又一个客服窗口 |
| ⚖️ **仪式感随困境伸缩** | 小纠结两个声音三轮就收，重大抉择五声部辩满五轮——不对小事过度戏剧化，不对大事敷衍了事 |
| 🛟 **安全不是免责声明** | 内置危机监测，刻意区分"直接危机信号"与"高强度痛苦表达"——后者不触发干预（不把激烈情绪当病），前者立即弹出求助资源。**不误伤**与**不漏接**同样重要 |

### ⚙️ 工程上 —— ⭐ 快模型 + 强 harness，可能比强模型更好
...
打开前端 → 🗣️ **对话**：把困境说给访谈者 → ⚖️ 系统评估复杂度、生成**冲突地图**并让你确认**内心画像** → 🔥 旁观内心声音的**辩论**（实时流式，议会偶尔会反过来问你）→ 🗺️ 得到**综合图景**（张力星图）→ 可继续**反思对话**或下载**收束报告**。

---

<div align="center">
<sub>Council of Me · 听见每一个自己 — 给自己一次旁观内心的机会 🌌</sub>
</div>
