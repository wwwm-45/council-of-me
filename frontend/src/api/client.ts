...
export interface ReflectionSelectOption {
  id: string;
  label: string;
  agent_name?: string;
  content?: string;
...
export interface ReflectionQuestion {
  ...
  question: string;
  hint: string;
  input_type: 'text' | 'multi_select' | 'slider_group';
  optional: boolean;
  options?: ReflectionSelectOption[];
  context?: {
    protective_intents?: ProtectiveIntent[];
    detected_frames?: DetectedFrame[];
    alternative_frame?: AlternativeFrame;
  };
}
...
export interface ReflectionMeta {
  ...
  has_framework_data: boolean;
}

export interface ReflectionPromptsResponse {
  r1: ReflectionQuestion[];
  r2: ReflectionQuestion[];
  r3: ReflectionQuestion[];
  r4: ReflectionQuestion[];
  meta: ReflectionMeta;
}
...
export const getReflectionPrompts = (id: string) =>
  request<ReflectionPromptsResponse>(`/sessions/${id}/reflection/prompts`);

export const postReflection = (id: string, level: string, responses: QuestionResponse[]) =>
  request<{ ok: boolean }>(`/sessions/${id}/reflection`, {
    method: 'POST',
